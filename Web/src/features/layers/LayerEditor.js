// LayerEditor.js - Fixed to handle wiper layer with correct name
...
const LayerEditor = () => {
  ...
  // Fixed: Check if this is the wiper layer (name is "Wiper", not "Wiper Layer")
  const isWiperLayer = layer.name === "Wiper" && layer.isWiperLayer === true
  
  const allowModelSelection = model.selectGroup !== "import" && !isWiperLayer
  ...
  const handleChange = (attrs) => {
    // Prevent changes to wiper layer name
    if (isWiperLayer && attrs.name !== undefined) {
      return
    }
    attrs.id = layer.id
  ...
  const renderOption = ({
    ...
    // Disable name editing for wiper layer
    const disabled = isWiperLayer && optionKey === "name"
  ...
  return (
    <div className="overflow-visible flex-grow-1">
      <div className="px-3 pt-3 border-top border-secondary">
        ...
        {!isWiperLayer && renderOption({ optionKey: "connectionMethod" })}
      </div>
      {model.canTransform(layer) && !isWiperLayer && (
