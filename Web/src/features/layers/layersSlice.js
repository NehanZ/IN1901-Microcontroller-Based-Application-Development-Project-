...
// Create default layers: Wiper first, then Fancy Text
const createDefaultLayers = () => {
  const wiperId = uuidv4()
  const fancyTextId = uuidv4()
  
  const wiperLayer = new Layer("wiper")
  const wiperState = {
    id: wiperId,
    name: "Wiper",
    isWiperLayer: true,
    ...wiperLayer.getInitialState(),
  }
  
  const fancyTextLayer = new Layer("fancyText")
  const fancyTextState = {
    id: fancyTextId,
    name: "Fancy Text",
    ...fancyTextLayer.getInitialState(),
  }
  
  return {
    entities: {
      [wiperId]: wiperState,
      [fancyTextId]: fancyTextState,
    },
    ids: [wiperId, fancyTextId],
    defaultId: fancyTextId, // Select fancy text by default
  }
}
...
const layersSlice = createSlice({
  ...
  initialState: adapter.getInitialState({
    current: defaultLayers.defaultId,
    selected: defaultLayers.defaultId,
    entities: defaultLayers.entities,
    ids: defaultLayers.ids,
  }),
  reducers: {
    addLayer: {
    ...
    deleteLayer: (state, action) => {
      ...
      const layer = state.entities[layerId]
      
      // Prevent deletion of wiper layer or if only 2 layers remain
      if (layer?.isWiperLayer || state.ids.length <= 2) {
        return
      }
    ...
    moveLayer: (state, action) => {
      ...
      const layerId = state.ids[oldIndex]
      const layer = state.entities[layerId]
      
      // Prevent moving wiper layer
      if (layer?.isWiperLayer) {
        return
      }
    ...
    updateLayer: (state, action) => {
      ...
      // Prevent updates to wiper layer name and certain properties
      if (layer?.isWiperLayer) {
        const allowedChanges = { ...changes }
        delete allowedChanges.name
        delete allowedChanges.type
        
        if (Object.keys(allowedChanges).length <= 1) { // Only id left
          return
        }
        
        const shape = getShape(layer.type)
        shape.handleUpdate(layer, allowedChanges)
        adapter.updateOne(state, { id: changes.id, changes: allowedChanges })
        return
      }
    ...
    addEffect: (state, action) => {
      ...
      // Prevent adding effects to wiper layer
      if (layer?.isWiperLayer) {
        return
      }
      
      const effectIds = layer.effectIds
      let index
      if (afterId) {
    ...
    moveEffect: (state, action) => {
      ...
      // Prevent moving effects on wiper layer
      if (layer?.isWiperLayer) {
        return
      }
    ...
    removeEffect: (state, action) => {
      ...
      // Prevent removing effects from wiper layer
      if (layer?.isWiperLayer) {
        return
      }
    ...
    changeModelType: (state, action) => {
      ...
      // Prevent changing wiper layer type
      if (layer?.isWiperLayer) {
        return
      }
    ...
    randomizeValues: (state, action) => {
      ...
      // Prevent randomizing wiper layer
      if (layer?.isWiperLayer) {
        return
      }
    ...
    restoreDefaults: (state, action) => {
      ...
      // Prevent restoring defaults for wiper layer
      if (layer?.isWiperLayer) {
        return
      }
      
      const { type, name, effectIds } = layer
      const layerInstance = new Layer(type)
...
export const deleteLayer = (id) => {
  return (dispatch, getState) => {
    ...
    // Prevent deleting wiper layer or if only 2 layers remain
    if (layer?.isWiperLayer || ids.length <= 2) {
      return
    }
